# Vendored legacy implementation.
#
# This file is generated/maintained to match the original gameplay code.
# Keep it in sync with: src/GuessTheDuck.DuckyBoi_XD/game.py
...
temp_CardDeck: list[str] = []
CardDeck: list[str] | None = None
savefile_value = 0
...
def encode_save(json_str: str) -> bytes:
...
def decode_save(encoded_bytes: bytes) -> str:
...
def load_game():
    """loading save file - returns past game data"""
...
cardSuits = [
    f"{Colours.BLACK}♠ - Spades{Colours.RESET}",
    f"{Colours.RED}♦ - Diamonds{Colours.RESET}",
    f"{Colours.BLACK}♣ - Clubs{Colours.RESET}",
    f"{Colours.RED}♥ - Hearts{Colours.RESET}",
]
...
def LINE():
    ...
    print(
        f"{Colours.BOLD}{Colours.MAGENTA}|====================--------------------==========--------==========--------------------====================|{Colours.RESET}"
    )
...
def print_tw(sentence: str, type_delay: float = 0.01):
...
def CLI_SW():
    ...
    while True:
        ...
        if width < 110 or height < 30:
            ...
            spaces_WH2 = 44 * " "
            print(
                f"{height_top_bottom}{spaces_TS}{Colours.BOLD}Terminal size too small:{Colours.RESET}\n"
...
def card_loading(countvalue: int):
...
def start():
    ...
    print_tw(
        f"{Colours.BOLD}{Colours.YELLOW}A CLI Python game about guessing every single card in a regular playing card deck.\n"
        f"When you successfully guess a card, it will be removed from the deck{Colours.RESET}\n\n"
        "No, you are not guessing ducks. Yes, the title is misleading\nI just like ducks and I name every project with 'duck'\n",
        0.0005,
    )
    if savefile_value == 0:
...
def guessingcard():
    ...
    while True:
        ...
        while True:
            ...
            gc_bottom_mid = (
                f"{Colours.BG_WHITE}{Colours.BOLD}{card_colour}│   {suit_logo}{Colours.BG_WHITE}{Colours.BOLD}{card_colour} │{Colours.RESET}"
            )
...
def guess_resolution():
    ...
    drawn_card_rank = drawn_card[:-1]
    drawn_card_suit = drawn_card[-1]
    ...
    if len(CardDeck) == 0:
        keypress_value = 2
        output_print = (
            f"🎉 {Colours.GREEN}{Colours.BOLD}Congratulations, you have successfully guessed the entire deck of playing card{Colours.RESET} 🎉"
        )
    clear_screen()
    card_loading(2)
    CLI_SW()
